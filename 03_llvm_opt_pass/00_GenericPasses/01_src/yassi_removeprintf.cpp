...
char RemovePrintfPass::ID = 0;

RemovePrintfPass::RemovePrintfPass(): 
    BasePass(), llvm::ModulePass(RemovePrintfPass::ID) 
{
...
bool RemovePrintfPass::runOnModule(llvm::Module& M) 
{
            ...
            for(auto in = bb->begin(), instruction_end = bb->end(); in != instruction_end; ++in) {
                if(llvm::isa<llvm::CallInst>(in) ) {
                    ...
                    if(hasname) {
                        ...
                        if(fn_name == "printf") instr_to_rm.push_back(llvm::cast<llvm::Instruction>(in));
                    }
                }
            }
        }
