/**
 * @file yassi_changeassumefunction.cpp
 * @brief Optimization Pass Modify Assunptions for Yassi
 *
...
char ChangeAssumeFunctionPass::ID = 0;

ChangeAssumeFunctionPass::ChangeAssumeFunctionPass(): 
    InstrBase(),
    llvm::ModulePass(ChangeAssumeFunctionPass::ID) 
{
}
...
bool ChangeAssumeFunctionPass::runOnModule(llvm::Module& M) 
{
            ...
            for(auto in = bb->begin(), instruction_end = bb->end(); in != instruction_end; ++in) {
                if(llvm::isa<llvm::CallInst>(in)) {
                    ...
                    if(hasname) {
                        ...
                        if(fn_name.find(BACKEND_FN_VERIFIER_ASSUME) != std::string::npos) {
                            ...
                            llvm::FunctionType* ch_assume_type = llvm::TypeBuilder<void(char*), false>::get(M.getContext());
                            llvm::Function* ch_assume_fun = llvm::cast<llvm::Function>(M.getOrInsertFunction(BACKEND_FN_ASSUME, ch_assume_type));
                            ...
                            llvm::CallInst::Create(ch_assume_fun, params, "",llvm::cast<llvm::Instruction>(insertpos));
                        }
                    }
                }
            }
